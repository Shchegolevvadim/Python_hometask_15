...
logging.basicConfig(level=logging.INFO, filename='data_text.log', encoding='utf-8',format=FORMAT, style="{")
logger = logging.getLogger(__name__)
MONTH = ("янв", "фев", "мар", "апр", "мая", "июн", "июл", "авг", "сен", "окт", "ноя", "дек")
DAY =("пон","вто","сре","чет","пят","суб","вос")
MONTHLEN = (31, 28, 31, 30, 31, )
def data_text(text: str) -> datetime:
    try:
        count, day, month = text.split()
    except ValueError as e:
        logger.error("Ошибка введения даты")
        return None
    ...
    for d in range(1, count_days+1):
        ...
        if date.weekday() == day:
            ...
            if count == count_week:
                print(date)
    return (count, day, month, date)
print(data_text('3-й воскресенье февраля'))